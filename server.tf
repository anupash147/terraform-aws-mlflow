  ...
  container_definitions = jsonencode(concat([
    {
      ...
      entryPoint = ["sh", "-c"]
      command = [
        "/bin/sh -c \"mlflow server --host=0.0.0.0 --port=${local.service_port} --default-artifact-root=s3://${local.artifact_bucket_id}${var.artifact_bucket_path} --backend-store-uri=postgresql://${data.aws_rds_cluster.database.master_username}:`echo -n $DB_PASSWORD`@${data.aws_rds_cluster.database.endpoint}:${data.aws_rds_cluster.database.port}/${var.database} --gunicorn-opts '${var.gunicorn_opts}' \""
      ]
