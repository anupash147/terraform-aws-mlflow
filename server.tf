  ...
  container_definitions = jsonencode(concat([
    {
      ...
      entryPoint = ["sh", "-c"]
      command = [
        "/bin/sh -c \"mlflow server --host=0.0.0.0 --port=${local.service_port} --default-artifact-root=s3://${local.artifact_bucket_id}${var.artifact_bucket_path} --backend-store-uri=postgresql://${module.aurora_postgresql.cluster_master_username}:`echo -n $DB_PASSWORD`@${module.aurora_postgresql.cluster_endpoint}:${module.aurora_postgresql.cluster_port}/${module.aurora_postgresql.cluster_database_name} --gunicorn-opts '${var.gunicorn_opts}' \""
      ]
